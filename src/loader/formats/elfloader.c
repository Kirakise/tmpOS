...
int elf_validate_loaded(struct elf_header *header){
  return (elf_valid_signature(header) && elf_valid_class(header) &&
      elf_valid_encoding(header) && elf_has_program_header(header)) ? 0 : -EINFORMAT;
}
...
int elf_process_phdr_pt_load(struct elf_file *elf_file, struct elf32_phdr *phdr){
  ...
  uint32_t end_virt_addr = phdr->p_vaddr + phdr->p_filesz;
  if (elf_file->virtual_end_address <= (void *)end_virt_addr || elf_file->virtual_end_address == 0){
    elf_file->virtual_end_address = (void *)end_virt_addr;
